...
class MyDBSCAN:
    ...
    def set_stock_data(self):
        ...
        for symbol in stock_symbols:
            stock_data[symbol] = pd.read_csv(
                "./dataset/stock_data/" + symbol + "_data.csv",
                usecols=["Date", "Close"],
            )
    ...
    def clustering_with_quarterly_returns(self):
        for metric in self.financial_metrics:
            features = pd.DataFrame(
                {
                    "Mean_Quarterly_Returns": self.quarterly_returns_all_stocks.mean(
                        axis=1
                    ),
                    metric: self.balance_sheet_quarterly[metric]
                    .reindex(self.quarterly_returns_all_stocks.index)
                    .ffill(),
                }
            ).dropna()
            ...
            # Plotting
            sns.scatterplot(
                x=features["Mean_Quarterly_Returns"],
                y=features[metric].astype(float),
                hue=clusters,
                palette="viridis",
            )
            ...
            print(
                f"Metrics for {metric} - Silhouette: {silhouette}, Davies Bouldin: {davies_bouldin}, Calinski Harabasz: {calinski_harabasz}"
            )
    ...
    def volatility_feature_engineering(self):
            ...
            self.features = pd.DataFrame(
                daily_returns_pca, index=self.data.columns, columns=["PC1", "PC2"]
            )
    ...
    def get_elbow_plot(self):
        ...
        plt.plot(
            np.arange(self.features_scaled.shape[0]),
            np.sort(dist[:, k - 1]),
            linewidth=2,
            markersize=5,
        )
    ...
    def volatility_clustering_without_pca(self, range_n_clusters=None):
        ...
        for eps in range_n_clusters:
            ...
            print(
                "Calinski Harabasz", self.metrics_without_pca["calinski_harabasz"][-1]
            )
